...
export default function RateSettingsPanel({
  ...
  const [presenceTypes, setPresenceTypes] = useState(DEFAULT_PRESENCE_TYPES);
  ...
  const projectRateColumns = [
    ...
    },
    {
      ...
      dataIndex: "baseRate",
      key: "baseRate",
      render: (value: number | string | null, record: Project) => {
        const currentValue =
          projectRates[record.id]?.baseRate ?? Number(value ?? 0);
            ...
            onChange={(newValue) => {
              setProjectRates((prev) => ({
                ...
                [record.id]: {
                  ...
                  baseRate: newValue ?? 0,
                  reviewerRate:
                    prev[record.id]?.reviewerRate ??
                    Number(record.reviewerRate ?? 0),
                },
              }));
            }}
          />
    ...
    },
    {
      ...
      render: (value: number | string | null, record: Project) => {
        const currentValue =
          projectRates[record.id]?.reviewerRate ?? Number(value ?? 0);
            ...
            onChange={(newValue) => {
              setProjectRates((prev) => ({
                ...
                [record.id]: {
                  ...prev[record.id],
                  baseRate:
                    prev[record.id]?.baseRate ?? Number(record.baseRate ?? 0),
                  reviewerRate: newValue ?? 0,
                },
              }));
            }}
          />
  ...
  const leaveMutation = useMutation({
    mutationFn: async () => {
      const response = await client.api["leave-requests"].settings.$patch({
        json: {
          unusedLeavePolicy: leaveSettings.unusedLeavePolicy as
            | "CARRY_OVER"
            | "PAID_AS_WORKED",

          adminLeaveQuota: String(leaveSettings.adminLeaveQuota),

          collaboratorLeaveQuota: String(leaveSettings.collaboratorLeaveQuota),
        },
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || "Failed to update leave settings");
      }
  ...
  useEffect(() => {
    ...
    if (attendanceSettings.presenceSelectionEnabled !== undefined) {
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      await Promise.all(
        Object.entries(projectRates).map(([projectId, projectRate]) =>
          updateProjectMutation.mutateAsync({
            id: projectId,
            data: {
              baseRate: projectRate.baseRate,
              reviewerRate: projectRate.reviewerRate,
            },
          }),
        ),
      );
      ...
      message.success("All rates saved successfully");
    } catch (error) {
      message.error(
